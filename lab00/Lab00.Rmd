...
Notice the table of contents on the left-hand side. These will allow you to efficiently navigate through the document, and if you click on one, it will open up into more detailed sub-sections.

**Data 8 Students / Python Users** For Data 8 students working in R and R Studio will be new. You will find that the basic concepts from python will transfer over, and you just need to concentrate on figuring out the difference in syntax. For those students who are more familiar with python, there are special notes about how R commands compare to python commands that look like this:

> **_Python Users:_** An example of python note!

If you're coming from STAT 20, you can ignore the comments and code with a green background. 

**Stat 20 Students / R users** For Stat 20 students this will feel very familiar, but some of the programming concepts, like how to write your own function and for-loops may be new. (Data 8 students will be familiar with these concepts, but should focus on the syntax in R). Please *slowly* go over these sections and if you are not clear ask for help in lab or do some outside google searches to find further tutorials on these concepts. 

For additional R support, see https://epurdom.github.io/Stat131A/RSupport/RResources.html
...
We will use the web version for labs, since this makes it easier and quicker to get going during labs -- by clicking on the links to the lab we provide you on bcourses, it will \upload necessary additional files for the lab into your workspace. 

We give brief instructions below for both methods, but for following this lab (and future labs), you should use the web version. For other assignments, you may find it easier to use RStudio on your own computer. In fact, we strongly recommend for your own convenience that if you have your own computer you should download and install R and do assignments (other than the labs) on your own machine. 
...
To login in to the server, open the url for this lab [here](https://r.datahub.berkeley.edu/hub/user-redirect/git-pull?repo=https%3A%2F%2Fgithub.com%2Fepurdom%2FStat131Public&urlpath=rstudio). Once successfully authenticated, you will see the following RStudio interface. 

![_A screen shot of Rstudio on initial startup_](figures/RStudio.png)
The files you see in the lower left corner may look different, but you should see the folder "Stat131LabsPublic". This will be where the necessary files for the lab will be located (e.g. data files, code files, etc.). We will explore this more below.

## Desktop Version

The layout looks similar to that of the web version.  The installation of R and RStudio is straightforward, just as installing other applications in your computer. We strongly recommend for your own convenience that if you have your own computer, download and install R and do assignments (other than the labs) on your own machine. 
...
For this lab, change your working directory to where the uploaded lab 00 files should be:

`Stat131Public` -> `lab00`

<img src="figures/wd_after.png" width="272" height="232" />


After doing this, you should see that the tab in the lower right marked files should show the files that were automatically imported when you clicked on the link to start this session:

<img src="figures/wd_filesAfter.png" width="356" height="178" />

These are the files you will be using in this lab, and you can create more during this lab (this is your personal working space...)
...
For example, open the R script  `example.R` in the lab 0 folder (under Stat131APublic/2019Fall/Labs/lab00) and try to run the code (you don't have to understand the code). You can run the code by either cutting and pasting into the console, or highlighting the code and clicking `Run` button. 
...
After changing your working directory, you should see the files that were imported
...
- *To load an installed package for current use* Once a package is installed, you have to "load" it to you current working environment in order to be able to actually use it. You can check box in front of geometry or type `library("geometry")` in the command window.

> **_Python Users:_** In python you would bring in modules with the command `import` to bring in other code; packages or libraries are the equivalent idea in R, and the command `library` is like that of `import`. A big difference is that in python you can *also* use `import` to bring in arbitrary code that you have written that is saved in a script file (e.g. functions you have written and saved in a `.py` file). In R the `library` command is *only* for bringing in bundled packages; this means mean code that has been developed and distributed in a particular format and has been installed into your R system on your computer. To read in arbitrary code in a script `.R` file you would use the function `source`. 
...
While R Scripts contains only R code, R Markdown enable us to create dynamic documents with embedded chunks of R code. Meaning you can write text and also have code that you want to describe in the same document. The R Markdown document can be converted to other formats such as HTML and PDF to have a "published" version. So you can develop a published report with the code embedded within it, ensuring that you can always recreate your work. This is opposed to, for example, cutting and pasting figures or results into a text editor like Word. 
...
<img src="figures/create_RMarkdown.png" width="325" height="261" />

You should now have a document which looks like this:

![](figures/Rmd_initiate.png)

RStudio has already populated the file with example text and code to get you started (which you would obviously delete for a real assignment). The code is contained within "chunks", which start with ` ```{r} ` and end with ` ``` `  (each on their own line):

![](figures/Rmd_default_code_chunk.png)

Extra options can be added in the ` ```{r} ` code. In the above example, this code chunk has been given a descriptive name ("cars"), via ` ```{r cars} ` 
...
If you click on the `run` icon at the top right of the chunk (green triangle), the entire chunk will be run and the output will either show the results below
...
<img src="figures/Rstudio_rchunk_render.png" width="150" height="200" />
...
To generate an HTML file of the entire document, click on the icon ![](figures/Rmd_genrate_HTML.png). 
...
Compiling the entire document will run all the commands from the beginning in a fresh environment. You will often discover mistakes when you do this, even if you think you have been testing your code interactively the whole time. Running everything fresh ensures that you have *reproducible* code, meaning someone else can start with just this document and recreate everything you show in your report. 

For example, suppose your code ran fine, you changed something, but didn't hit run. Or if you move a chunk around and don't realize that you moved it *after* another chunk that depends on the results of that chunk -- if you run the chunks in the right order, you won't get an error. But when you compile it from top to bottom, it won't work. 

You should always give yourself time to test your assignment before the due date. Furthermore, it is wise as you finish one section/question of your assignment to compile the entire document and see if you get what you expect, rather than waiting until you are completely finished.
...
<img src="figures/Rnb_preview_button.png" width="246" height="46" />
...
<img src="figures/Rnb_nbhtml_runcode.png" width="331" height="367" />
...
<img src="figures/Rnb_preview_dropdown.png" width="178" height="161" />
...
This tutorial is compiled from a Rmarkdown document, but is a little bit complicated as an introduction to Rmarkdown because of the screenshots and python comments that we have added. 

In future labs, your labs will be Rmarkdown documents. You will open them up and interact with the code directly in the Rmarkdown file we have given you as part of the lab.  You will turn in your modified Rmarkdown, along with a compiled pdf version as your solution to gradescope. 
...
In what follows, we will move away from Rmarkdown, and just show you the R code. You can type this directly into the console, or (better!) use the R script document we were working with above. 
...
You can use either one, and you should be comfortable reading code with either one. 
...
```{python class.source="bg-success", eval=FALSE,python.reticulate=FALSE}
# python code:
import math
import numpy as np
...
**Exercise 1.** You've seen the probability density function of Normal distribution $N(\mu, \sigma^2)$ in Data8 or Stat 20 as follows:
...
**Exercise 2** We want to use R and the function `dnorm` that evaluates the normal density to answer the following questions. Recall that `dnorm(x,mean=1,sd=2)` is the value of the normal density with mean 1 and standard deviation 2 evaluated at x. 
...
sig.sq <- 2
x <- 0
# Add code with result saved as `result1`
...
```{r}
# Add code with result saved as `result2`
...
*Note:* For those that are familiar with vector operations this means that `x*y` for vectors in R is NOT the inner product, but element wise, and ditto for matrices. 
...
Notice how R treats the variable differently now that it's a factor -- the function `summary` gives entirely different results for the factor vector as it does the character vector. In fact, `summary` for a factor works like the function `table`, which simply counts the number of times each unique entry is present.
