...
`Stat131Public` -> `2019Fall` -> `Labs` -> `lab00`
...
1. Random sampling: We can sample with and without replacement from a set of values with the function `sample`. We give the function a vector of values we want to sample from, the size of the sample we want, and whether we want to sample with or without replacement. We also use the argument `set.seed` so that in fact our random numbers will be the same every time we rerun this code, rather than varying (we will discuss this more in Lab 1).
...
*Example 1* Use `inner_join()` to merge `twitter_info` and `twitter_follows` data frames. 
```{r}
twitter = twitter_info %>%
  inner_join(twitter_follows, by = 'Screen.name')
twitter
```

*Example 2*: Use `group_by()` and `summarise()` to get numbers of actors/actress, average followers and friends for each medium type 
```{r}
head(twitter)
twitter %>% 
  group_by(Medium) %>%
  summarise(n_male = sum(Gender == 'M'),
            n_female = sum(Gender == 'F'),
            avg_followers = mean(Followers),
            avg_friends = mean(Friends))
```

*Example 3*: Use `mutate()` and `strsplit()` (from `base`) to create columns with first name and last names. Then use `select()` to re-organize columns.
```{r}
twitter %>%
  mutate(First = strsplit(Name,' ')[[1]][1],
         Last = strsplit(Name,' ')[[1]][2]) %>%
  select(Screen.name, Name, First, Last, Gender, Medium, Followers, Friends)
```

## Examples working with ggplot2

*Example 4*: Use `geom_histogram()` and `geom_vline()` to reproduce the histogram above.
```{r}
# First, we need to set up data frames for mean and median values
baby_lines = data.frame(x = c(mean(baby$Birth.Weight), median(baby$Birth.Weight)),
                        linetype = c('Mean','Median'),
                        values = c(1,2))
ggplot(baby, aes(x = Birth.Weight)) +
  geom_histogram(color = 'black', fill = 'blue',bins = 14) +
  # add vertical lines for mean and median
  geom_vline(aes(xintercept = x, linetype = linetype),baby_lines, color = 'red') +
  # manually assign different linetypes for mean and medians 
  scale_linetype_manual(values = baby_lines$values) +
  labs(x = 'Birth.Weight (in ounces)', y = 'Frequency', linetype = '') +
  theme_bw() # setting black and white background for the plot
  
```

*Example 5*: Use ggplot functions to reproduce scatterplots above
```{r}
ggplot(baby, aes(x = Maternal.Pregnancy.Weight, y = Birth.Weight)) +
  geom_point(color = 'dark blue', # setting color, fill, and alpha for all points
             fill = 'dark blue',
             alpha = 0.5) +
  labs(x = 'Maternal.Pregnancy.Weight (in lbs)',
...
```{r}
baby = baby %>% 
  mutate(ageBinned = cut(Maternal.Age, c(0,21,35,100)))
ggplot(baby, aes(x = Maternal.Pregnancy.Weight, y = Birth.Weight,
                 color = ageBinned)) + # setting different colors for different
                                       # age bins
  geom_point(alpha = 0.5) +
  labs(x = 'Maternal.Pregnancy.Weight (in lbs)',
       y = 'Birth.Weight (in ounces)') +
  scale_color_brewer(palette = 'Set2') + # using color palette
  theme_bw()
```

```{r}
baby = baby %>%
  mutate(whLowBirth = Birth.Weight < 60)
ggplot(baby, aes(x = Maternal.Pregnancy.Weight, y = Birth.Weight,
                 color = whLowBirth, size = whLowBirth)) + # setting different colors 
                                                           # and sizes for groups of 
                                                           # low birth weight babies
  geom_point(alpha = 0.5) +
  scale_size_manual(values = c(1,5))+
  labs(x = 'Maternal.Pregnancy.Weight (in lbs)',
       y = 'Birth.Weight (in ounces)',
       title = 'Highlighting in Red Low Weight Babies') +
